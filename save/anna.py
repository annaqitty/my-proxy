...
def error(*args):
    print(red + "[" + yellow + "!" + red + "] - " + defcol + " ".join(map(str, args)))
        ...
        while True:
            ...
            try:
                if not url.startswith("http://") and not url.startswith("https://"):
                    url = "http://" + url
                get = urllib.request.urlopen(url, timeout=10)  # Timeout set to 10 seconds
                ...
                error("HTTP Error:", e)
                if e.code == 403:
                    error("Forbidden Error (403): The server refused to fulfill the request.")
                elif e.code == 429:
                    error("Too Many Requests Error (429): The server received too many requests.")
